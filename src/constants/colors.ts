// const COLORS = {
// 	background: 'rgba(129, 212, 250, 1)',
// 	black: 'rgba(0, 0, 0, 1)',
// 	blue: 'rgba(0, 0, 255, 1)',
// 	containerBackground: 'rgba(97, 212, 250, 1)',
// 	red: 'rgba(255, 0, 0, 1)',
// 	transparent: 'rgba(0, 0, 0, 0.7)',
// 	white: 'rgba(255, 255, 255, 1)',
// };

// export default COLORS;

// export const COLORS2 = {
// 	background: 'rgba(129, 212, 250, 1)', // Светло-голубой (энергия, свежесть)
// 	black: 'rgba(33, 33, 33, 1)', // Тёмный серый
// 	blue: 'rgba(41, 98, 255, 1)', // Насыщенный синий
// 	red: 'rgba(211, 47, 47, 1)', // Красный (ошибки, удаление)
// 	transparent: 'rgba(0, 0, 0, 0.7)', // Полупрозрачный чёрный
// 	white: 'rgba(255, 255, 255, 1)', // Белый
// 	accent: 'rgba(0, 200, 83, 1)', // Ярко-зелёный (успех, кнопки)
// };

// export const COLORS3 = {
// 	background: 'rgba(28, 28, 30, 1)', // Dark Gray - #1C1C1E
// 	primary: 'rgba(63, 81, 181, 1)', // Indigo - #3F51B5
// 	primaryAlt: 'rgba(30, 136, 229, 1)', // Blue - #1E88E5
// 	accent: 'rgba(0, 229, 255, 1)', // Neon Blue - #00E5FF
// 	white: 'rgba(255, 255, 255, 1)',
// 	textPrimary: 'rgba(255, 255, 255, 0.95)',
// 	textSecondary: 'rgba(255, 255, 255, 0.6)',
// 	border: 'rgba(255, 255, 255, 0.12)',
// 	overlay: 'rgba(0, 0, 0, 0.7)',
// };

// export const COLORS4 = {
// 	// Base
// 	background: 'rgba(129, 212, 250, 1)', // Soft light blue
// 	text: 'rgba(255, 255, 255, 1)', // Global text color (white)

// 	// Primary Action
// 	primary: 'rgba(3, 169, 244, 1)', // Action blue
// 	primaryDark: 'rgba(2, 136, 209, 1)', // On press
// 	primaryLight: 'rgba(129, 212, 250, 1)', // Light tint (optional)
// 	grey: 'rgba(0, 0, 0, 0.35)',

// 	// States
// 	success: 'rgba(102, 187, 106, 1)', // Green - done
// 	red: 'rgba(239, 83, 80, 1)', // Red - error
// 	warning: 'rgba(255, 213, 79, 1)', // Yellow - caution
// 	info: 'rgba(100, 181, 246, 1)', // Info - subtle light blue

// 	// Surfaces
// 	card11: 'rgba(255, 255, 255, 0.08)', // Card-like block
// 	overlay11: 'rgba(0, 0, 0, 0.2)', // Dim background
// 	modal11: 'rgba(0, 0, 0, 0.4)', // Full-screen modal overlay

// 	// Borders & Lines
// 	border: 'rgba(255, 255, 255, 0.2)',
// 	divider: 'rgba(255, 255, 255, 0.1)',

// 	// Disabled
// 	disabled: 'rgba(255, 255, 255, 0.5)',
// 	disabledBg: 'rgba(255, 255, 255, 0.2)',

// 	// Feedback elements
// 	shadowLight: 'rgba(255, 255, 255, 0.3)',
// 	shadowDark: 'rgba(0, 0, 0, 0.3)',

// 	// Optional brand variants
// 	accentPink: 'rgba(236, 64, 122, 1)',
// 	accentPurple: 'rgba(171, 71, 188, 1)',
// 	accentOrange: 'rgba(255, 112, 67, 1)',

// 	// Transparent
// 	transparent: 'rgba(0, 0, 0, 0)',
// };

const COLORS = {
	white: 'rgba(255, 255, 255, 1)',
	black: 'rgba(0, 0, 0, 1)',
	green: 'rgba(102, 187, 106, 1)',
	yellow: 'rgba(255, 213, 79, 1)',
	red: 'rgba(239, 83, 80, 1)',
	transparent: 'rgba(0, 0, 0, 0.7)',

	grey: 'rgba(0, 0, 0, 0.4)',

	background: 'rgba(139, 212, 250, 1)', //FAVOURITE

	// primaryText: 'rgba(255, 255, 255, 1)',
	// secondaryText: 'rgba(255, 255, 255, 0.75)',
	// secondaryText2: 'rgba(41, 98, 255, 1)',
	// secondaryText3: 'rgba(3, 169, 244, 1)',

	// textButton2: 'rgba(0, 122, 255, 1),',
	textButton: 'rgba(30, 136, 229, 1)',

	// icon: 'rgba(85, 85, 85, 1)',

	baseButtonBGC: 'rgba(3, 169, 244, 1)',
	// baseButtonBGC2: 'rgba(41, 98, 255, 1)',

	// baseButtonText: 'rgba(255, 255, 255, 1)',

	// inputBGC: 'rgba(255, 255, 255, 1)',
	// inputText: 'rgba(26, 26, 26, 1)',

	// backgroundSoft: 'rgba(179, 229, 252, 1)',
	// backgroundSky: 'rgba(144, 202, 249, 1)',
	// backgroundLight: 'rgba(240, 248, 255, 1)',

	// background1: 'rgba(179, 229, 252, 1)', // NORM BGC
	// backgroundAlt: 'rgba(144, 202, 249, 1)', // NORM BGC

	// background2: 'rgba(63, 81, 181, 1)', // Indigo - #3F51B5
	// background3: 'rgba(30, 136, 229, 1)', // Blue - #1E88E5
	// background4: 'rgba(0, 229, 255, 1)',
	// background5: 'rgba(41, 98, 255, 1)',
	// background6: 'rgba(0, 122, 255, 1),',
	// background7: 'rgba(3, 169, 244, 1)',

	// background8: 'rgba(2, 136, 209, 1)', // On press
	// background9: 'rgba(33, 150, 243, 1)',
	// background10: 'rgba(240, 248, 255, 1)',
	// background11: 'rgba(100, 181, 246, 1)', // Info - subtle light blue
	// background12: 'rgba(97, 212, 250, 1)',
};

export default COLORS;

// export const COLORS6 = {
// 	background1: 'rgba(120, 81, 169, 1)', // Medium purple (soft but rich)
// 	background: 'rgba(123, 104, 238, 1)',

// 	primaryText: 'rgba(255, 255, 255, 1)', // White for main text on purple bg
// 	secondaryText: 'rgba(149, 117, 205, 1)', // Light lavender purple for secondary text

// 	textButton1: 'rgba(123, 104, 238, 1)', // Medium slate blue for links/buttons
// 	textButton2: 'rgba(120, 81, 169, 1)',

// 	icon: 'rgba(85, 85, 85, 1)', // Neutral dark grey for icons

// 	baseButtonBGC: 'rgba(98, 0, 238, 1)', // Vivid purple for buttons
// 	baseButtonText: 'rgba(255, 255, 255, 1)', // White text on buttons

// 	inputBGC: 'rgba(245, 240, 255, 1)', // Very pale lavender background for inputs
// 	inputText: 'rgba(26, 26, 26, 1)', // Dark text in inputs
// };
